    ...
    if not just_demo:
        ...
        while x >= 0:
            userId = users[random.randint
                                 (0, len(users) -1)].id
            notebooklist = [notebook.id for notebook in Notebook.query.filter_by(user_id = userId)] 
            noteslist=[note for notebookid in notebooklist for note in Note.query.filter_by(notebook_id = notebookid)]
            if len(noteslist) >1:
