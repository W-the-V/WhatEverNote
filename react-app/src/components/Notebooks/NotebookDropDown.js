...
const NotebookDropDown = ({userId, setShowActions, Notebook}) => {
    ...
    const addNewNote =async () => {
        const defaultNote = {Title: "Default Note", Text: "<p>Start writing your note</p>", notebook_id: Notebook.id}
        let newNote = await dispatch(createNote(defaultNote, userId))
        history.push(`/notes/${newNote.id}`)
    }
    const dispatch = useDispatch()
    const RenameNoteBook = async (e) => {
        e.preventDefault()
        const updatedNotebook = {id:Notebook.id, name:newName,default_notebook:Notebook.default_notebook, user_id: userId}
    ...
    const SetAsDefaultNotebook = async () => {
        const oldDefaultNoteBook = notebooksList.filter(notebook => notebook.default_notebook)[0]
        const updatedNotebook = {id:Notebook.id, name:Notebook.name, default_notebook:true, user_id: userId}
        let old = await dispatch(updateNotebooks({id:oldDefaultNoteBook.id, default_notebook:false, name:oldDefaultNoteBook.name, user_id:userId}))
    ...
    return (
        <div className="NotebookDropDown__container">
            ...
            >
                <div className="Notebook-action__container">
                    ...
                    <form onSubmit={(e)=>DeleteNoteBook(e)}>
                        ...
                        <span id="close" onClick={()=>setShowDeleteWarning(false)}>X</span>
                        </div>
            ...
            <span onClick={()=>setShowActions(false)} id="close">X</span>
            <ul>
